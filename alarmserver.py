...
class AlarmServerConfig():
    def __init__(self, configfile):
        ...
        # We have some values that we may want to get from the environment. They will have a special value of "ENV"
        if self.ALARMCODE == 'ENV':
            self.ALARMCODE = os.environ['ALARMCODE']
        if self.CALLBACKURL_BASE == 'ENV':
            self.CALLBACKURL_BASE = os.environ['CALLBACKURL_BASE']
        if self.CALLBACKURL_APP_ID == 'ENV':
            self.CALLBACKURL_APP_ID = os.environ['CALLBACKURL_APP_ID']
        if self.CALLBACKURL_ACCESS_TOKEN == 'ENV':
